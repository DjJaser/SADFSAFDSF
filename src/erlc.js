...
function buildApiUrl(endpoint) {
  const base = String(config.erlcApiBaseUrl || "").trim().replace(/\/+$/, "");
...
export async function fetchKillLogs() {
  if (!config.erlcApiBaseUrl || !config.erlcApiKey) {
    console.warn("ER:LC API config is not complete. Skipping kill log fetch.");
    return [];
  }

  const endpoint = buildApiUrl(config.erlcKillLogsEndpoint);
  const response = await fetch(endpoint, {
    method: "GET",
    headers: {
      Accept: "*/*",
  ...
  if (!response.ok) {
    ...
    throw new Error(`ER:LC killlogs failed with status ${response.status}: ${body}`);
  }
  ...
  if (Array.isArray(payload?.KillLogs)) {
    return payload.KillLogs;
  }
...
export async function fetchActiveVehicles() {
  if (!config.erlcApiBaseUrl || !config.erlcApiKey) {
    console.warn("ER:LC API config is not complete. Skipping vehicle fetch.");
    return [];
  }

  const endpoint = buildApiUrl(config.erlcVehiclesEndpoint);
  const response = await fetch(endpoint, {
    method: "GET",
    headers: {
      Accept: "*/*",
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => "");
    throw new Error(`ER:LC vehicles failed with status ${response.status}: ${body}`);
  }
...
export async function fetchServerPlayers() {
  if (!config.erlcApiBaseUrl || !config.erlcApiKey) {
    console.warn("ER:LC API config is not complete. Skipping players fetch.");
    return [];
  }

  const endpoint = buildApiUrl(config.erlcPlayersEndpoint);
  const response = await fetch(endpoint, {
    method: "GET",
    headers: {
      Accept: "*/*",
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => "");
    throw new Error(`ER:LC players failed with status ${response.status}: ${body}`);
  }
...
export async function fetchEmergencyCalls() {
  if (!config.erlcApiBaseUrl || !config.erlcApiKey) {
    console.warn("ER:LC API config is not complete. Skipping emergency calls fetch.");
    return [];
  }

  const endpoint = buildApiUrl(config.erlcEmergencyCallsEndpoint);
  const response = await fetch(endpoint, {
    method: "GET",
    headers: {
      Accept: "*/*",
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => "");
    throw new Error(`ER:LC emergency calls failed with status ${response.status}: ${body}`);
  }
  ...
  if (Array.isArray(payload?.emergencyCalls)) {
    return payload.emergencyCalls;
  }
