...
function normalizeVehicleLookupKey(value) {
  return normalizeVehicleName(value).replace(/[^A-Z0-9]/g, "");
}
...
export function registerVehicleName(vehicleName) {
  ...
  writeStore(store);
  return prettifyVehicleName(vehicleName);
}
...
export function addOwnedVehicle(userId, vehicleName, metadata = {}) {
  return updateAccount(userId, (current) => {
    current.cars ??= {};
    const normalized = normalizeVehicleName(vehicleName);
    current.cars[normalized] = {
      name: prettifyVehicleName(vehicleName),
...
export function userOwnsVehicle(userId, vehicleName) {
  ...
  return Object.entries(account.cars ?? {}).some(([storedKey, record]) => {
    ...
    return areVehicleNamesEquivalent(record?.name, vehicleName);
  });
}
