...
async function processWebsiteCarPurchase({
  ...
  const updated = updateAccount(account.discordUserId, (current) => {
    ...
    current.cars ??= {};
    current.cars[normalizeVehicleName(vehicleToStore)] = {
      name: vehicleToStore,
  ...
  const vehicleStoredSuccessfully = Boolean(
    updated?.cars?.[normalizeVehicleName(vehicleToStore)]
  ) && (
  ...
  if (!updated || !afterAccount || !vehicleStoredSuccessfully) {
    console.error("[WEBSITE BUY CAR STORE FAILED]", JSON.stringify({
      ...
      cleanVehicleName,
      vehicleToStore,
      updatedHasVehicle: Boolean(updated?.cars?.[normalizeVehicleName(vehicleToStore)]),
