...
function canManageProjectRecord(member, project) {
  return ["owner", "partner", "admin", "employee"].includes(getProjectAccessLevel(member, project));
}

function canUseProjectAction(member, project, action) {
  const level = getProjectAccessLevel(member, project);

  if (level === "owner" || level === "partner") {
    return true;
  }
...
function buildWebsiteAccountSnapshot(account) {
  ...
  return {
    ...
    ownedCarsCount: Object.keys(account.cars ?? {}).length,
    ownedCars: Object.keys(account.cars ?? {})
  };
}
...
async function processWebsiteCarPurchase({
  ...
  addOwnedVehicle(account.discordUserId, cleanVehicleName, {
  ...
  await sendAuditLog(client, config.auditChannelId, {
    ...
    fields: [
      ...
      { name: "🚘 **المركبة**", value: `**${cleanVehicleName}**`, inline: true },
...
function normalizeWeaponInventoryEntry(entry = {}, weaponKey = "m9") {
  return {
    ...
    permanent: Boolean(entry.permanent) || !entry.expiresAt,
...
function buildWeaponStatusText(entry) {
  ...
  return `مؤقت • ينتهي <t:${Math.floor(new Date(entry.expiresAt).getTime() / 1000)}:R>`;
}
...
function hasPermanentWeapon(account, weaponKey) {
  return getWeaponInventory(account, weaponKey).some((entry) => (entry.active !== false && !entry.brokenAt) && (entry.permanent || !entry.expiresAt));
}
...
function buildWeaponOperationDmEmbed({
  ...
  const details = [
    ...
    `**📜 النوع:** ${permanent || !expiresAt ? "دائم" : "مؤقت"}`,
...
async function processExpiredWeapons() {
  const guild = await client.guilds.fetch(config.guildId).catch(() => null);
  if (!guild) {
    return;
  }
...
async function pollActiveVehicles() {
  ...
  for (const entry of activeVehicles) {
    ...
    if (shouldSkipVehiclePunishment(parsed.vehicleName, ownerTeam)) {
      continue;
    }
    ...
    registerVehicleName(parsed.vehicleName);
    invalidateVehicleCatalogCache();

    const uniqueKey = `${parsed.id}|${normalizeVehicleName(parsed.vehicleName)}|${normalizeVehicleName(ownerUsername)}`;
    const lastHandledAt = processedVehicleIds.get(uniqueKey) || 0;
    if (Date.now() - lastHandledAt < 30 * 1000) {
      continue;
    }

    if (isFreeVehicleName(parsed.vehicleName, ownerTeam)) {
      processedVehicleIds.set(uniqueKey, Date.now());
      continue;
    }

    const linkedAccount = findAccountByRobloxUsername(ownerUsername);
    const ownsVehicle = linkedAccount?.discordUserId
      ? userOwnsVehicle(linkedAccount.discordUserId, parsed.vehicleName)
      : false;
    ...
    await sendVehicleEnforcementAudit({
      ...
      allowed: false,
      reason: linkedAccount ? "المركبة غير موجودة ضمن ممتلكاته" : "لا يوجد حساب بنكي أو ملكية مسجلة"
    });
  }
...
function createCraftingWeaponMenu(levelKey = "level1") {
  ...
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId("crafting_weapon_select")
...
function buildCraftingHomeEmbed(account) {
  ...
  return applyCraftingImage(new EmbedBuilder()
    ...
    .setDescription([
      ...
      "",
      "**📋 التعليمات:**",
      "• المستوى الأول فقط متاح حاليًا للشراء",
      "• المستويات الأعلى قيمتها مجهولة وغير متاحة",
...
function buildWeaponRemovedDmEmbed({ weaponCode, weaponLabel, permanent, expiresAt, executorLabel }) {
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "📜 **النوع**", value: `**${permanent || !expiresAt ? "دائم" : "مؤقت"}**`, inline: true },
  ...
  try {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "weapon_info_menu") {
      ...
      if (action === "give_weapon") {
        await interaction.showModal(createWeaponTransferModal());
        return;
      }

      await interaction.reply({ content: "الخدمة المطلوبة غير معروفة.", ephemeral: true });
      return;
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "crafting_level_select") {
      ...
      const level = CRAFTING_TABLE_LEVELS[levelKey];
      if (!level) {
        await interaction.reply({ content: "مستوى الطاولة غير صالح.", ephemeral: true });
        return;
      }
      ...
      await interaction.reply({
        ...
        files: level.purchasable ? getCraftingImageFiles() : [],
        ephemeral: true
      });
      return;
    }

    if (interaction.isStringSelectMenu() && interaction.customId === "crafting_weapon_select") {
      await interaction.deferReply({ ephemeral: true });

      const account = requireAccount(interaction.user.id);
      const availableWeaponKeys = new Set(getCraftingWeaponsForLevel("level1").map((weapon) => weapon.key));
      const hasLevel1Role = interaction.member?.roles?.cache?.has(CRAFTING_LEVEL1_ACCESS_ROLE_ID);
      if (!account || (!hasCraftingTable(account, 1) && !hasLevel1Role)) {
        await interaction.editReply({ content: "أنت لا تملك طاولة تصنيع مستوى أول أو رتبة الوصول الخاصة بها." });
        return;
      }
      ...
      if (weaponKey === "refresh") {
        await interaction.editReply({
          embeds: [buildCraftingLevelEmbed("level1", account)],
          components: [createCraftingWeaponMenu("level1")],
          files: getCraftingImageFiles()
        });
        return;
      }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "resource_shop_menu") {
      ...
      await interaction.showModal(createResourceModal(selected));
      return;
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "bank_menu") {
      ...
      if (action === BANK_ACTIONS.transfer) {
        ...
        await interaction.showModal(createTransferModal());
        return;
      }
      ...
      if (action === BANK_ACTIONS.loan) {
        ...
        await interaction.showModal(createLoanModal(account.accountNumber));
        return;
      }
      ...
      if (action === BANK_ACTIONS.payFine) {
        ...
        await interaction.showModal(createPayFineModal());
        return;
      }
      ...
      if (action === BANK_ACTIONS.removeHold) {
        ...
        await interaction.showModal(createRemoveHoldRequestModal());
        return;
      }
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId === CAR_ACTIONS.confirmPurchase) {
        ...
        if (!vehicleName) {
          await interaction.reply({ content: "انتهت صلاحية طلب الشراء. اختر المركبة من جديد.", ephemeral: true });
          return;
        }
        ...
        if (userOwnsVehicle(interaction.user.id, vehicleName)) {
          pendingCarPurchases.delete(interaction.user.id);
          await interaction.reply({ content: "هذه المركبة مسجلة لديك بالفعل.", ephemeral: true });
          return;
        }

        const offer = getVehicleOffer(vehicleName);
        if (!offer.isFree && account.balance < offer.price) {
          await interaction.reply({ content: "رصيدك البنكي لا يكفي لشراء هذه المركبة.", ephemeral: true });
          return;
        }
      ...
      if (interaction.customId.startsWith("crafting_dm_approve:")) {
        ...
        if (!result.ok) {
          ...
          await interaction.reply({ content: "تعذر بدء اختبار التصنيع الآن.", ephemeral: true });
          return;
        }
      ...
      if (interaction.customId.startsWith("crafting_confirm_weapon:")) {
        ...
        const weapon = CRAFTABLE_WEAPONS[weaponKey];
        const account = requireAccount(interaction.user.id);
        const availableWeaponKeys = new Set(getCraftingWeaponsForLevel("level1").map((item) => item.key));
        ...
        if (!hasCraftingTable(account, 1) && !hasLevel1Role) {
          await interaction.editReply({ content: "لا يمكنك التصنيع من طاولة المستوى الأول إلا إذا كنت تملك الطاولة أو رتبة الوصول الخاصة بها." });
          return;
        }

        if (!hasLevel1Role) {
          await interaction.editReply({
            content: "لا يمكنك التصنيع من طاولة المستوى الأول إلا بعد الحصول على رتبة التصنيع الخاصة بها."
          });
          return;
        }
        ...
        updateAccount(interaction.user.id, (current) => {
          ...
          const slotIndex = appendWeaponInventory(current, weaponKey, {
            ...
            expiresAt: null,
            permanent: true,
            source: "crafting_level1",
            weaponLabel: weapon.label
          });
          craftedWeaponCode = buildWeaponInventoryCode(weaponKey, slotIndex);
        ...
        await sendPoliceBankLog({
          ...
          fields: [
            ...
            { name: "🔫 **السلاح**", value: `**${weapon.label}**`, inline: true },
            { name: "🆔 **رمز النسخة**", value: `**${craftedWeaponCode}**`, inline: true },
            { name: "📜 **النوع**", value: "**دائم**", inline: true }
          ]
        });
        ...
        await interaction.user.send({
          embeds: [
            buildWeaponOperationDmEmbed({
              title: "🛠️ تم تصنيع سلاح M9 دائم",
              description: "تم تصنيع نسخة جديدة من سلاحك بنجاح عبر طاولة المستوى الأول.",
      ...
      if (interaction.customId.startsWith("transfer_confirm:")) {
        ...
        if (!transfer) {
          await interaction.reply({ content: "عملية التحويل لم تعد متاحة.", ephemeral: true });
          return;
        }
        ...
        if (!senderAccount || !targetAccount) {
          await interaction.reply({ content: "تعذر إتمام التحويل لعدم وجود حساب.", ephemeral: true });
          return;
        }
        if (isAccountBankFrozen(senderAccount) || isAccountBankFrozen(targetAccount)) {
          await interaction.reply({ content: "تعذر تنفيذ التحويل لأن أحد الحسابين مجمّد حكوميًا.", ephemeral: true });
          return;
        }

        if (senderAccount.balance < transfer.amount) {
          await interaction.reply({ content: "رصيدك لم يعد كافيًا لإتمام التحويل.", ephemeral: true });
          return;
        }
      ...
      if (interaction.customId.startsWith("finepay_approve:")) {
        ...
        if (!fine || !account || fine.targetUserId !== interaction.user.id || fine.status === "paid") {
          await interaction.reply({ content: "تعذر إتمام سداد المخالفة.", ephemeral: true });
          return;
        }

        if (account.balance < fine.amount) {
          await interaction.reply({ content: "فشلت عملية السداد، رصيدك لا يكفي.", ephemeral: true });
          return;
        }
        ...
        await interaction.update({ content: "تم سداد المخالفة بنجاح.", embeds: [], components: [] });
    ...
    if (interaction.type === InteractionType.ModalSubmit) {
      ...
      if (interaction.customId === "modal_confirm_bank_pin") {
        ...
        if (!pending) {
          await interaction.reply({ content: "لا يوجد طلب إنشاء حساب معلق.", ephemeral: true });
          return;
        }

        const enteredPin = interaction.fields.getTextInputValue("bank_pin");
        if (pending.pin !== enteredPin) {
          await interaction.reply({ content: "الرقم السري غير صحيح.", ephemeral: true });
          return;
        }
      ...
      if (interaction.customId === "modal_bank_transfer") {
        ...
        if (!targetUser) {
          await interaction.reply({ content: "حساب الشخص غير موجود.", ephemeral: true });
          return;
        }

        if (interaction.user.id === targetUser.id) {
          await interaction.reply({ content: "لا يمكنك التحويل لنفسك.", ephemeral: true });
          return;
        }
        ...
        if (isAccountBankFrozen(targetAccount)) {
          await interaction.reply({ content: "لا يمكن التحويل إلى هذا الحساب لأنه مجمّد.", ephemeral: true });
          return;
        }

        if (senderAccount.balance < amount) {
          await interaction.reply({ content: "رصيدك أقل من المبلغ المطلوب.", ephemeral: true });
          return;
        }
      ...
      if (interaction.customId === "modal_remove_hold_request") {
        ...
        await interaction.reply({ content: "تم إرسال طلب رفع إيقاف الخدمات.", ephemeral: true });
        return;
      }
      ...
      if (interaction.customId === "modal_bank_loan") {
        ...
        if (account.accountNumber !== accountNumber) {
          await interaction.reply({ content: "رقم الحساب البنكي غير مطابق.", ephemeral: true });
          return;
        }

        if (!Number.isFinite(amount) || amount <= 0) {
          await interaction.reply({ content: "مبلغ القرض غير صالح.", ephemeral: true });
          return;
        }
        ...
        await interaction.reply({ content: "تم إرسال طلب القرض إلى الإدارة.", ephemeral: true });
        return;
      }
...
client.once(Events.ClientReady, async (readyClient) => {
  ...
  await processExpiredWeapons().catch((error) => {
