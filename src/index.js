...
async function processWebsiteCarPurchase({
  ...
  const ownedVehicleRecord = addOwnedVehicle(account.discordUserId, vehicleToStore, {
    purchasePrice: finalPrice,
    grantedBy: "website",
    source: sourceLabel
  });
  if (!ownedVehicleRecord) {
    if (updated) {
  ...
  const vehicleStoredSuccessfully = afterAccount?.discordUserId
    ? userOwnsVehicle(afterAccount.discordUserId, vehicleToStore)
    : false;

  if (!afterAccount || !vehicleStoredSuccessfully) {
    if (afterAccount?.discordUserId && !vehicleStoredSuccessfully) {
      updateAccount(account.discordUserId, (current) => {
        current.balance += finalPrice;
        current.cars ??= {};
