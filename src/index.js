...
async function processWebsiteCarPurchase({
  ...
  addOwnedVehicle(account.discordUserId, vehicleToStore, {
...
async function pollActiveVehicles() {
  ...
  for (const entry of activeVehicles) {
    ...
    if (!linkedAccount?.discordUserId) {
      console.log("[VEHICLE CHECK SKIP] Unable to resolve owner account with certainty:", JSON.stringify({
        ownerUsername,
        vehicleNameFromApi: parsed.vehicleName,
        canonicalVehicleName,
        ownerTeam
      }));
      processedVehicleIds.set(uniqueKey, Date.now());
    ...
    const ownsVehicle = linkedAccount?.discordUserId
      ? userOwnsVehicle(linkedAccount.discordUserId, canonicalVehicleName)
      : false;
    ...
    await sendVehicleEnforcementAudit({
      ...
      allowed: false,
      reason: "المركبة غير موجودة ضمن ممتلكاته بعد التحقق من الحساب المرتبط"
    });
  }
